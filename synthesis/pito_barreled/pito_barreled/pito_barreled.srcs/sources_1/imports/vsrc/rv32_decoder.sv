        ...
        case (instr[6:2])
            ...
            5'b00100: begin // ADDI, SLTI, SLTIU, XORI, ORI, ANDI, SLLI, SRLI, SRAI, NOP
                case ({instr[30], instr[14:12]})
                    {1'b0, 3'b000}  : begin rv_opcode <= RV32_ADDI;  rv_alu_op <= `ALU_ADD; end
                    {1'b0, 3'b001}  : begin rv_opcode <= RV32_SLLI;  rv_alu_op <= `ALU_SLL; end
                    {1'b0, 3'b010}  : begin rv_opcode <= RV32_SLTI;  rv_alu_op <= `ALU_SLT; end 
                    {1'b0, 3'b011}  : begin rv_opcode <= RV32_SLTIU; rv_alu_op <= `ALU_SLTU;end 
                    {1'b0, 3'b100}  : begin rv_opcode <= RV32_XORI;  rv_alu_op <= `ALU_XOR; end 
                    {1'b0, 3'b101}  : begin rv_opcode <= RV32_SRLI;  rv_alu_op <= `ALU_SRL; end 
                    {1'b1, 3'b101}  : begin rv_opcode <= RV32_SRAI;  rv_alu_op <= `ALU_SRA; end
                    {1'b0, 3'b110}  : begin rv_opcode <= RV32_ORI;   rv_alu_op <= `ALU_OR;  end
                    {1'b0, 3'b111}  : begin rv_opcode <= RV32_ANDI;  rv_alu_op <= `ALU_AND; end
                endcase
            end
